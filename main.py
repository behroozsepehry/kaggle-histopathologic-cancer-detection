...
def create_submission(model, dataloader, device, settings):
    ...
    result_list = n_util.apply_func_to_model_data(model, lambda out, inp: (out, inp[1]), dataloader, device)
    result_labels = np.concatenate([x[0].cpu().squeeze(1).numpy() for x in result_list])
    result_ids = []
    for x in result_list:
        result_ids += x[1]
    result_ids = [os.path.basename(x).split('.')[0] for x in result_ids]

    submission_path = settings.get('submission_path')
    if submission_path:
        df_sub = pd.DataFrame({'id': result_ids, 'label': result_labels})
        df_sub.to_csv(submission_path, header=True, index=False)

    return result_labels, result_ids
